...
export const Toolbar = ({
  ...
  <header className="flex items-start justify-between gap-8 border-b border-border bg-[#fafafa] px-8 py-6">
    <div className="min-w-0">
      ...
      <div className="mt-2 flex min-w-0 items-center gap-3 text-sm text-muted-foreground">
        <span className="h-3 w-3 rounded-sm bg-[#40AEF0] ring-1 ring-black/10" />
        <span className="truncate font-mono">{schemaDirectoryPath ?? "未加载 CS 文件夹"}</span>
      </div>
    </div>
    <div className="flex shrink-0 flex-col items-end gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" onClick={onOpenSchemaDirectory}>
          <FolderOpen className="h-4 w-4" />
          加载 CS 文件夹
        </Button>
        <Button variant="outline" onClick={onSave} disabled={!filePath && !dirty}>
          <Save className="h-4 w-4" />
          保存
        </Button>
        <Button variant="outline" onClick={onSaveAs} disabled={!filePath && !dirty}>
